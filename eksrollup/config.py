...
app_config = {
    ...
    'K8S_CONTEXT': os.getenv('K8S_CONTEXT', 'ara-staging-use1'),
    'K8S_PROXY_BYPASS': str_to_bool(os.getenv('K8S_PROXY_BYPASS', False)),
    'K8S_WORKER_GROUPS_ORDER': os.getenv('K8S_WORKER_GROUPS_ORDER', '["redis", "deployments", "plugins", "monitoring"]'),
    ...
    'EXCLUDE_NODE_LABEL_KEYS': os.getenv('EXCLUDE_NODE_LABEL_KEYS', 'ara.app/tier=storage').split(),
    'EXTRA_DRAIN_ARGS': os.getenv('EXTRA_DRAIN_ARGS', '').split(),
    'MAX_ALLOWABLE_NODE_AGE': int(os.getenv('MAX_ALLOWABLE_NODE_AGE', 5)),
